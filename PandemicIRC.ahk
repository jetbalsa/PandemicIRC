...
{
	MsgBox, There was a problem reading your Settings.ini file. Please run the Configure tool!
	ExitApp
}
Server := Settings.Server
Nicks := StrSplit(Server.Nicks, ",", " `t")

Process,Close,stunnel.exe
Run, %A_ScriptDir%\stunnel\bin\stunnel.exe
...
Chat.SetOptions(["AUTOVSCROLL"], "Set")
...
GuiControl, Move, Chat, x5 y5 w%EditW% h%EditH%
...
class Bot extends IRC
{
	...
	{
		IniRead, TTS, Settings.ini, Alert, TTS, false
		IniRead, AntiSpam, Settings.ini, Alert, AntiSpam, false
		IniRead, Beep, Settings.ini, Alert, Beep, false
		IniRead, LastText, Settings.ini, Alert, LastText, false
		AppendChat("-" NickColor(Nick) "- " Msg)
		if(AntiSpam){
		if(LastText = Msg){
		return
		}
		IniWrite, %Msg%, Settings.ini, Alert, LastText
		}
		Notify("Pandemic IRC Notice","-" Nick "- " Msg,15,"AC=RestoreTray IN=8 IW=48 IH=48")
		if(TTS){
			SetTimer, RunTTS, -1
		}
		if(Beep){
...
WM_SYSCOMMAND(wParam)
{
   If (wParam = 61472) ; minimize
      SetTimer, MinimizeTray, -1
   Else If (wParam = 61728) ; restore
      SetTimer, RestoreTray, -1
}
...
MinimizeTray:
   ...
   Gui, Hide
   Menu, Tray, Icon
   TrayTip, Pandemic Super Simple IRC, Pandemic IRC has been minized to the tray`nClick Icon to restore, 20, 17
   ;Notify("Pandemic Super Simple IRC","I'm sitting in your icon try awaiting commands. Click to show",3,"AC=RestoreTray IN=8 IW=48 IH=48")
Return

RunTTS:
Thread, interrupt, 0
IniRead, LastText, Settings.ini, Alert, LastText, false
ComObjCreate("SAPI.SpVoice").Speak(LastText)
Return
